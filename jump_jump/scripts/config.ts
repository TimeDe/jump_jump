...
import { CustomPlugin, CopyIndex } from './myplugin';
...
const config: ResourceManagerConfig = {
    ...
    buildConfig: (params) => {
        ...
        if (command == 'build') {
            ...
            return {
                ...
                commands: [
                    new EmitResConfigFilePlugin({
                        ...
                        groupSelector: p => groupSelector(p)
                    }),
            ...
            let outputDir;
            if (version.indexOf("AI") > -1) {
                let v = version.split("AI");
                outputDir = `bin-release/AI/${v.join("")}/http/game`;
                return {
                    outputDir,
                    commands: [
                ...
                outputDir = `bin-release/web/${projectName}`;
                return {
                    outputDir,
                    commands: [
                        new CompilePlugin({ libraryType: "release", defines: { DEBUG: false, RELEASE: true } }),
...
function groupSelector(path): string | null {
    if (path.indexOf(".mp3") > -1) {
        return 'music';
    } else if (path.indexOf('/loadingImg/') > -1) {
        return "loading";
    } else if (path.indexOf('.exml') > -1) {
        return null;
    } else {
        return 'preload';
    }
